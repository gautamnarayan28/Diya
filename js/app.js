...
(function () {
  ...
  /* ---------------- audio: pre-recorded clips (Sarvam) with browser TTS fallback ----------------
     ...
     Clip keys: "name", "ing-N", "step-N"; file = audio/<recipeId>/<clipKey>.mp3 (Hindi only for now).
  ...
  function recipeClipKeys(r) {
    const keys = ["name"];
    r.ingredients.forEach((ing, i) => { if (!ing.group && hasClip(r.id, `ing-${i}`)) keys.push(`ing-${i}`); });
    r.steps.forEach((_, i) => keys.push(`step-${i}`));
    return keys;
  }
